...
export function TournamentWizard() {
  ...
  const addCompetitor = () => {
    ...
    setNewName(""); setNewImg(undefined);
  };
  ...
  const create = () => {
    ...
    let t: Tournament = {
      ...
      league: { pointsWin, pointsDraw, pointsLoss },
    };
    if (format === "liga") {
      ...
      {step === 2 && (
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="glass space-y-4 rounded-2xl p-6">
          ...
          <div className="flex flex-wrap items-end gap-2">
            ...
            <button onClick={addCompetitor} className="rounded-lg bg-primary px-4 py-2 text-sm font-semibold">+ Agregar</button>
          </div>

          <div className="grid gap-2 sm:grid-cols-2">
            {comps.map((c, i) => (
              <div key={c.id} className="flex items-center gap-3 rounded-lg border border-border bg-secondary/20 p-2">
                ...
                <button onClick={() => setComps((cs) => cs.filter((_, j) => j !== i))} className="text-xs text-destructive">×</button>
              </div>
            ))}
          </div>
